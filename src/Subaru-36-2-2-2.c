...
/** @brief 主 RPM 中断服务程序（斯巴鲁 36-2-2-2 解码器）
 *
 * 处理斯巴鲁 36-2-2-2 曲轴轮信号，用于斯巴鲁发动机的位置检测和 RPM 计算。
 * 36-2-2-2 表示：36 个齿，减去 3 组各 2 个齿（共 6 个缺齿），用于同步和相位识别。
 * 
 * @details 为什么需要这个方法：
 * 斯巴鲁发动机使用特殊的曲轴轮模式（36-2-2-2），这种模式：
 * - 提供精确的曲轴位置信息
 * - 通过 3 组缺齿（每组 2 个）实现同步和相位识别
 * - 支持顺序喷油和精确点火控制
 * - 适用于斯巴鲁水平对置发动机
 * 
 * 当前状态：
...
/** @brief 次 RPM 中断服务程序（斯巴鲁 36-2-2-2 解码器）
 *
 * 处理次 RPM 输入（通常是凸轮轴信号），用于区分发动机循环和确定相位。
 * 在斯巴鲁 36-2-2-2 解码器中，次 RPM 输入用于确定发动机处于哪个 360 度循环中。
 * 
 * @details 为什么需要这个方法：
