...
/** @brief 燃油和点火计算
 *
 * ECU 的核心计算函数，使用各种算法计算基础燃油脉宽，然后应用各种修正，
 * 最终得到每个气缸的喷油脉宽和点火提前角。这是连接传感器数据和执行器控制的关键桥梁。
 * 
 * @details 为什么需要这个方法：
 * ECU 需要将传感器数据转换为执行器控制信号：
 * 1. 输入：传感器数据（MAP、TPS、RPM、温度等）、查找表（VE、Lambda 等）
 * 2. 处理：计算基础脉宽、应用各种修正
 * 3. 输出：每个气缸的喷油脉宽、点火提前角、闭合角
 * 
 * 计算流程：
 * 1. 空气流量计算：
 *    - SpeedDensity（速度密度）：MAP × VE / 100%
 *    - AlphaN（节气门角度）：使用 TPS 和调参表
 *    - MAF（质量空气流量）：直接使用 MAF 传感器
 *    - FixedAF（固定空气流量）：用于测试
 * 
 * 2. 密度和燃油因子计算：
 *    - 考虑进气温度、Lambda（目标空燃比）、燃油密度
 *    - 用于将空气流量转换为燃油量
 * 
 * 3. 基础脉宽（BasePW）计算：
 *    - BasePW = (启动燃油常数 × 空气流量) / 密度和燃油因子
 * 
 * 4. 应用修正：
 *    - TFC（瞬态燃油修正）：加速/减速时的燃油修正
 *    - ETE（发动机温度修正）：冷启动时的加浓
 *    - PCFC（每缸修正）：各气缸的微调
 *    - IDT（喷油器死区时间）：根据电池电压查找，必须加到脉宽中
 * 
 * 5. 点火计算（待实现）：
 *    - 点火提前角：从查找表获取，应用各种修正
 *    - 闭合角：根据电池电压和 RPM 计算
 ...
 *    - 根据点火提前角和闭合角调度点火事件
 * 
 * @return 无返回值，计算结果直接写入全局变量：
 *         - injectorMainPulseWidthsMath[]: 各气缸的喷油脉宽
 *         - ignitionAdvances[]: 各气缸的点火提前角
 *         - currentDwellMath: 闭合角
 *
 * @author Fred Cooke
 * 
