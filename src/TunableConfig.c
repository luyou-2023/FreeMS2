...
/** @brief 可调配置 - 小表格A（Flash中的常量定义）
 *
 * 此结构体包含第一组可调配置数据中的小表格A部分。
...
const volatile SmallTables1 SmallTablesAFlash  TUNETABLESD = {
		...
		{ARRAY_OF_16_TEMPS, ARRAY_OF_16_ZEROS},       	/* engineTempEnrichmentTableFixed - 发动机温度加浓表（固定值）：根据发动机温度进行固定值加浓 */
		{ARRAY_OF_16_TEMPS, ARRAY_OF_16_ZEROS},       	/* primingVolumeTable - 预注油量表：冷启动时的预注油量，根据温度查找 */
		{ARRAY_OF_16_TEMPS,  ARRAY_OF_16_PERCENTS},      	/* engineTempEnrichmentTablePercent - 发动机温度加浓表（百分比）：根据发动机温度进行百分比加浓 */
...
/** @brief 可调配置 - 小表格B（Flash中的常量定义）
 *
 * 此结构体包含第一组可调配置数据中的小表格B部分。
 */
